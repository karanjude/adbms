...
public class ViewFriendReqProc {
	...
	public final String sql = new String(
			"SELECT userid, inviterid, inviteeid, username, fname, lname,gender, dob, jdate, ldate, address,email,tel "
					+ "FROM Users, Friendship WHERE (inviteeid=? and status = 1 and inviterid = userid)"
					+ "order by userid, inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel;");
	private final Connection conn;

	public ViewFriendReqProc(Connection conn) {
		this.conn = conn;
	}

	public int execute(int profileOwnerID,
