...
/**
 * Reads the log record files and assigns them to the validation threads for
 * being processed merges the stats it gets from the validation threads
 * 
 * @author barahman
 * 
 */
public class ValidationMainClass {
	...
	public static final String DB_TENANT_PROPERTY_DEFAULT = "single"; // or
																		// multi
	...
	public static final String VALIDATION_APPROACH_PROPERTY_DEFAULT = "interval"; // or
																					// RDBMS
	...
	public static void buildValidationIndexes(Properties props) {
		...
		String user = props.getProperty(VALIDATION_DBUSER_PROPERTY,
				VALIDATION_DBUSER_PROPERTY_DEFAULT);
		String passwd = props.getProperty(VALIDATION_DBPWD_PROPERTY,
		...
		int machineid = Integer
				.parseInt(props.getProperty(Client.MACHINE_ID_PROPERTY,
		...
		try {
			int count = 1;
			if (props.getProperty(DB_TENANT_PROPERTY,
					DB_TENANT_PROPERTY_DEFAULT).equalsIgnoreCase("single"))
				count = 1;
			else
				count = Integer.parseInt(props.getProperty(
						Client.THREAD_CNT_PROPERTY,
						Client.THREAD_CNT_PROPERTY_DEFAULT));

			stmt = conn.createStatement();
			long startIdx = System.currentTimeMillis();
			for (int i = 1; i <= count; i++) {
				dropIndex(stmt, "TUPDATE" + machineid + "c" + i
						+ "_IDX$$_start");
				...
				stmt.executeUpdate("CREATE INDEX TUPDATE" + machineid + "c" + i
						+ "_IDX$$_start ON TUPDATE" + machineid + "c" + i
						+ " (STARTTIME)" + "COMPUTE STATISTICS NOLOGGING");
				stmt.executeUpdate("CREATE INDEX TUPDATE" + machineid + "c" + i
						+ "_IDX$$_end ON TUPDATE" + machineid + "c" + i
						+ " (ENDTIME)" + "COMPUTE STATISTICS NOLOGGING");
				stmt.executeUpdate("CREATE INDEX TUPDATE" + machineid + "c" + i
						+ "_IDX$$_resource ON TUPDATE" + machineid + "c" + i
						+ " (RID)" + "COMPUTE STATISTICS NOLOGGING");
				stmt.executeUpdate("CREATE INDEX TUPDATE" + machineid + "c" + i
						+ "_IDX$$_optype ON TUPDATE" + machineid + "c" + i
						+ " (OPTYPE)" + "COMPUTE STATISTICS NOLOGGING");

				stmt.executeUpdate("analyze table tupdate" + machineid + "c"
						+ i + " compute statistics");
				long endIdx = System.currentTimeMillis();
				System.out.println("\t Time to build validation index for"
						+ machineid + " structures(ms):" + (endIdx - startIdx));
	...
	public static void dumpFilesAndValidate(Properties props,
			HashMap<Integer, Vector<Integer>> seqTracker,
			HashMap<Integer, Vector<Integer>> staleSeqTracker,
			HashMap<String, Integer> stalenessStats, PrintWriter outpS,
			String dir) {

		// open files for all threads one by one and read the records into the
		// memory
		...
		int tableId = 1; // if single tenant they will all dump into tread1 and
							// tupdate1
		...
		int machineid = Integer
				.parseInt(props.getProperty(Client.MACHINE_ID_PROPERTY,
						Client.MACHINE_ID_PROPERTY_DEFAULT));
		String ratingMode = props.getProperty(Client.RATING_MODE_PROPERTY,
				Client.RATING_MODE_PROPERTY_DEFAULT);
		String tenant = props.getProperty(DB_TENANT_PROPERTY,
				DB_TENANT_PROPERTY_DEFAULT);
		int threadCount = Integer
				.parseInt(props.getProperty(Client.THREAD_CNT_PROPERTY,
						Client.THREAD_CNT_PROPERTY_DEFAULT));
		...
		try {
			...
			for (int i = 0; i < threadCount; i++) {
				fstream = new FileInputStream(dir + "//update" + machineid
						+ "-" + i + ".txt");
				...
				while ((line = br.readLine()) != null) {
					numUpdates++;
					tokens = line.split(",");
					logObject record = new logObject(tokens[0], tokens[1],
							tokens[2], tokens[3], tokens[4], tokens[5],
							tokens[6], tokens[7], tokens[8]);
					if (updateStats.get(record.getMopType() + "-"
							+ record.getRid()) != null) {
						resourceUpdateStat newVal = updateStats.get(record
								.getMopType() + "-" + record.getRid());
						String tempValMinS = updateStats.get(
								record.getMopType() + "-" + record.getRid())
								.getMinStartTime();
						String tempValMaxE = updateStats.get(
								record.getMopType() + "-" + record.getRid())
								.getMaxEndTime();
						String tempValV = updateStats.get(
								record.getMopType() + "-" + record.getRid())
								.getFinalVal();

						// update min start time if needed
						if (Long.parseLong(tempValMinS) > Long.parseLong(record
								.getStarttime()))
						...
						if (Long.parseLong(tempValMaxE) < Long.parseLong(record
								.getEndtime()))
						...
						/*
						 * update structure
						 * updateStats.put(record.getMopType()+"-"
						 * +record.getRid(), newVal);
						 */
					} else {
					...
					if (approach.equalsIgnoreCase("RDBMS")) {
						...
						sqlStr = "INSERT INTO tupdate"
								+ machineid
								+ "c"
								+ tableId
								+ " (opType, seqid, threadid, rid, starttime, endtime, numofupdate, updatetype) VALUES ("
								+ "'" + record.getMopType() + "', "
								+ record.getSeqId() + ", "
								+ record.getThreadId() + ", " + record.getRid()
								+ ", " + record.getStarttime() + ", "
								+ record.getEndtime() + ", "
								+ record.getValue() + ", '"
						...
						updateStats.get(
								record.getMopType() + "-" + record.getRid())
								.addInterval(
										Long.parseLong(record.getStarttime()),
										Long.parseLong(record.getEndtime()),
										updateTypeInLong);
					}
				}
				in.close();
			...
			System.out.println("\t-- Done reading update files...elapsed time:"
					+ (updateReadEnd - updateReadStart));
			if (approach.equalsIgnoreCase("RDBMS")) {
			...
			// discard the reads that don have updates on their resources or
			// those that happened before first update on their resources, or
			// those happened after last update
			// read all read files
			int readToValidate = Integer.parseInt(props.getProperty(
					VALIDATION_BLOCK_PROPERTY,
					VALIDATION_BLOCK_PROPERTY_DEFAULT));
			int threadsToValidate = Integer.parseInt(props.getProperty(
					VALIDATION_THREADS_PROPERTY,
					VALIDATION_THREADS_PROPERTY_DEFAULT));
			int numReadOpsProcessed = 0; // these are the ones that were not
											// pruned and needed to be processed
			...
			for (int i = 0; i < threadCount; i++) {
				fstream = new FileInputStream(dir + "//read" + machineid + "-"
						+ i + ".txt");
				DataInputStream in = new DataInputStream(fstream);
				BufferedReader br = new BufferedReader(
						new InputStreamReader(in));
				...
				while ((line = br.readLine()) != null) {
					...
					if (seqTracker.get(Integer.parseInt(record.getThreadId())) == null) {
						...
						seqTracker.put(Integer.parseInt(record.getThreadId()),
								valLst);
					} else if (!seqTracker.get(
							Integer.parseInt(record.getThreadId())).contains(
							Integer.parseInt(record.getSeqId()))) {
						Vector<Integer> valLst = seqTracker.get(Integer
								.parseInt(record.getThreadId()));
						valLst.add(Integer.parseInt(record.getSeqId()));
						seqTracker.put(Integer.parseInt(record.getThreadId()),
								valLst);
					}
					...
					if (updateStats.get(record.getMopType() + "-"
							+ record.getRid()) != null) {
						resourceUpdateStat updateLogStat = updateStats
								.get(record.getMopType() + "-"
										+ record.getRid());
						// check if read before first update
						if (Long.parseLong(record.getEndtime()) < Long
								.parseLong(updateLogStat.getMinStartTime())) {
							pruned++;
							int cmpVal = 0;
							if (initCnt.get(record.getMopType() + "-"
									+ record.getRid()) == null)
								cmpVal = 0;
							else
								cmpVal = initCnt.get(record.getMopType() + "-"
										+ record.getRid());
							if (Integer.parseInt(record.getValue()) != cmpVal) {
								numStaleReadsReturned++;
								System.out
										.println("before update case: Data was stale for "
												+ record.getMopType()
												+ ": "
												+ record.getSeqId()
												+ "-"
												...
												+ " value Read is="
												+ record.getValue());
								if (staleSeqTracker.get(Integer.parseInt(record
										.getThreadId())) == null) {
									Vector<Integer> valLst = new Vector<Integer>();
									valLst.add(Integer.parseInt(record
											.getSeqId()));
									staleSeqTracker.put(Integer.parseInt(record
											.getThreadId()), valLst);
								} else if (!staleSeqTracker.get(
										Integer.parseInt(record.getThreadId()))
										.contains(
												Integer.parseInt(record
														.getSeqId()))) {
									Vector<Integer> valLst = staleSeqTracker
											.get(Integer.parseInt(record
													.getThreadId()));
									valLst.add(Integer.parseInt(record
											.getSeqId()));
									staleSeqTracker.put(Integer.parseInt(record
						...
						} else if (Long.parseLong(record.getStarttime()) > Long
								.parseLong(updateLogStat.getMaxEndTime())) {
							...
							if (initCnt.get(record.getMopType() + "-"
								...
								inVal = initCnt.get(record.getMopType() + "-"
										+ record.getRid());
							if (Integer.parseInt(record.getValue()) != (Integer
									.parseInt(updateLogStat.getFinalVal()) + inVal)) {
								numStaleReadsReturned++;
								System.out
										.println("after update case: Data was stale for "
												+ record.getMopType()
												+ ": "
												+ record.getSeqId()
												+ "-"
							...
							if (readToValidate == toBeProcessed.size()) {
								// we have all elements ready in vector to be
								// processed
								if (vThreads.size() == threadsToValidate) {
									...
									for (Thread t : vThreads) {
										try {
											t.join();
											numReadOpsProcessed += ((ValidationThread) t)
													.getNumOps();
											numStaleReadsReturned += ((ValidationThread) t)
													.getNumStaleOps();
											validationTime += ((ValidationThread) t)
													.getValidationTime();
											HashMap<Integer, Vector<Integer>> staleSeqReturned = ((ValidationThread) t)
													.getStaleSequences();
											Set<Integer> tids = staleSeqReturned
													.keySet();
											Iterator<Integer> it = tids
													.iterator();
											it = tids.iterator();
											while (it.hasNext()) {
												Integer tid = (Integer) it
														.next();
												// get all the stale seqs seen
												// by this thread
												Vector<Integer> seqsSeen = staleSeqReturned
														.get(tid);
												for (int j = 0; j < seqsSeen
														.size(); j++) {
													if (staleSeqTracker
															.get(tid) != null) {
														if (!staleSeqTracker
															...
															// TODO: check this
															// (
															// (Vector<Integer>)staleSeqTracker.get(tid)
															// ).add(seqsSeen.get(j));
															Vector<Integer> vals = staleSeqTracker
																	.get(tid);
															vals.add(seqsSeen
																	.get(j));
															staleSeqTracker
								...
								ValidationThread newVThread = new ValidationThread(
										props, toBeProcessed, updateStats,
										initCnt);
						...
						if (null != record.getValue()
								&& !record.getValue().equalsIgnoreCase("null")
								&& Integer.parseInt(record.getValue()) != cmpVal) {
							numStaleReadsReturned++;
							System.out
									.println("never updated case: Data was stale for "
											+ record.getMopType()
											+ ": "
											+ record.getSeqId()
											+ "-"
							...
							if (staleSeqTracker.get(Integer.parseInt(record
								...
								staleSeqTracker.put(
										Integer.parseInt(record.getThreadId()),
										valLst);
							} else if (!staleSeqTracker
									.get(Integer.parseInt(record.getThreadId()))
									.contains(
											Integer.parseInt(record.getSeqId()))) {
								Vector<Integer> valLst = staleSeqTracker
										.get(Integer.parseInt(record
			...
			if (toBeProcessed.size() > 0) {
				ValidationThread newVThread = new ValidationThread(props,
						toBeProcessed, updateStats, initCnt);
				vThreads.add(newVThread);
				newVThread.start();
				for (int u = 0; u < vThreads.size(); u++) {
					vThreads.get(u).join();
					numReadOpsProcessed += ((ValidationThread) vThreads.get(u))
							.getNumOps();
					numStaleReadsReturned += ((ValidationThread) vThreads
							.get(u)).getNumStaleOps();
					validationTime += ((ValidationThread) vThreads.get(u))
							.getValidationTime();
					HashMap<Integer, Vector<Integer>> staleSeqReturned = ((ValidationThread) vThreads
							.get(u)).getStaleSequences();
						...
						if (staleSeqReturned.get(tid) != null) {
							...
							for (int j = 0; j < seqsSeen.size(); j++) {
								if (staleSeqTracker.get(tid) != null) {
									if (!staleSeqTracker.get(tid).contains(
											seqsSeen.get(j))) {
										Vector<Integer> vals = staleSeqTracker
												.get(tid);
			...
			System.out.println("ReadValidationDuration(ms):"
					+ (updateReadEnd - updateReadStart));
			// populate statistics
			...
			System.out
					.println("\t TotalReadOps = "
							+ (numReadOpsProcessed + pruned)
							+ " ,staleReadOps="
							+ numStaleReadsReturned
							+ " ,staleness Perc (gran:user)="
			...
			System.out.println("\t TotalSeqRead = " + totalSeq
					+ " ,staleSeqRead=" + totalStaleSeq
					+ " ,staleness Perc (gran:user)="
	...
	public static void createValidationSchema(Properties props) {
		...
		try {
			stmt = conn.createStatement();
			int count = 1;
			if (tenant.equalsIgnoreCase("single")) // create one read1 and one
													// write1 table
				count = 1;
			else
				count = threadCount; // create a read and a write table per
										// thread

			for (int i = 1; i <= count; i++) {
				dropSequence(stmt, "UPDATECNT" + machineid + "c" + i);
				dropTable(stmt, "TUPDATE" + machineid + "c" + i);
				stmt.executeUpdate("CREATE SEQUENCE  UPDATECNT"
						+ machineid
						+ "c"
						+ i
						+ "  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER  NOCYCLE");
				stmt.executeUpdate("CREATE TABLE TUPDATE"
						+ machineid
						+ "c"
						+ i
				...
				stmt.executeUpdate("ALTER TABLE TUPDATE" + machineid + "c" + i
						+ " MODIFY (UPDATEID NOT NULL ENABLE)");
				stmt.executeUpdate("ALTER TABLE TUPDATE" + machineid + "c" + i
						+ " MODIFY (THREADID NOT NULL ENABLE)");
				stmt.executeUpdate("ALTER TABLE TUPDATE" + machineid + "c" + i
						+ " MODIFY (RID NOT NULL ENABLE)");

				stmt.executeUpdate("CREATE OR REPLACE TRIGGER UPDATEINC"
						+ machineid + "c" + i + " before insert on tupdate"
						+ machineid + "c" + i + " " + "for each row "
						+ "WHEN (new.updateid is null) begin "
						+ "select updateCnt" + i
						+ ".nextval into :new.updateid from dual;" + "end;");
				stmt.executeUpdate("ALTER TRIGGER UPDATEINC" + machineid + "c"
						+ i + " ENABLE");
			}
	...
	public static void main(String[] args) {
		...
		if (props.getProperty(VALIDATION_APPROACH_PROPERTY,
				VALIDATION_APPROACH_PROPERTY_DEFAULT).equals("RDBMS"))
			createValidationSchema(props);
		dumpFilesAndValidate(props, seqTracker, staleSeqTracker,
				stalenessStats, null, dir);
