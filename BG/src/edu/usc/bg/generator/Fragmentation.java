...
public class Fragmentation {
	...
	Cluster[] clusters;
	DistOfAccess myDist;
	//Vector<Integer> myMemberIds = new Vector<Integer>();
	int[] myMemberIds;
	int _machineid = 0;
	int _numShards = 0;
	...
	public Fragmentation(int usercount, int numBGClients, int machineid, String probs, double ZipfianMean){
		...
		for(int i=0; i<usercount; i++){
			...
			//System.out.println(i+" "+origProbs[i]);
		...
		for(int i =0; i< numClusters; i++){
			for(int j=0; j<clusters[i].members.size();j++){
				...
				normalizedProbs[clusters[i].members.get(j).getUserid()] = (((clusters[i].members.get(j).getOrigProb())/clusters[i].getTotalProb())*(props[i]/weight));
				clusterProbs[clusters[i].members.get(j).getUserid()] = ((clusters[i].members.get(j).getOrigProb())/clusters[i].getTotalProb());
			}
		}
		for(int i =0; i< numClusters; i++){
		...
		//compute the initial prob of the highest 20% of users
		...
		//compute the normalized prob of the highest 20% of users
		//need to sort the probs
		...
		for(int i=(sortedNormElements.length-1); i>=(sortedNormElements.length-interestedUserCount);i--){	
		...
		//myDist = new DistOfAccess(clusters[machineid].members.size(),"Zipfian",true,ZipfianMean);
		//System.out.println("%%%%%"+clusters[machineid].members.size());
		//rewrite the probs with the probs within each cluster
		...
		for(int t=0; t<clusters[machineid].members.size(); t++){
			...
			//System.out.println(clusters[machineid].members.get(t).getUserid()+" "+newProbs[t]);
		}
		//myDist.reWriteProbs(newProbs);
		myDist = dist;
	...
	public static void main(String[] args){
		...
		int myNumReqs = (numReqs/numBGClients);
		int remain = numReqs - (myNumReqs*numBGClients);
		if(i<remain) myNumReqs++;
		for(int j=0; j<myNumReqs; j++){
			int idx = myDist.GenerateOneItem()-1;
			int key = myMembers[idx];
			frequencies[key]++;
		}
		...
		try {
			...
			String fileName = "ADZipfian"+usercount+"-"+numBGClients+"-"+numReqs+".txt";
