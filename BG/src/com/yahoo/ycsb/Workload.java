...
public abstract class Workload
{
	 ...
	 * effects other than DB operations and mutations on threadstate. Mutations to threadstate do not need to be
	 * synchronized, since each thread has its own threadstate instance.
	 * @param updateLog is the update log record created by that transaction which needs to be written to the update file
	 * @param readLog is the read log record created by that transaction which needs to be written to the update file
	 ...
	 * @param thinkTime between the actions in a session
	 * @param insertImage , if set to true means there are images inserted in the database and they need to be retrieved for relevant actions i.e. getUserProfile
	...
	/**
	 * gets the initial stats from the data stores: member count, friend count per user, pending friend count per user and the resource count per user
